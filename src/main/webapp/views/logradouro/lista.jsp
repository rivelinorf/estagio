						...
						<c:forEach items="${tipologradouro.all}" var="tipologradouro">
							<c:choose>
								<c:when test="${tipologradouro.id == filtroLogradouro.tipologradouro.id}">
									<option value="${tipologradouro.id}" selected>${logradouro.nome}</option>
								</c:when>
								<c:otherwise>
				...
				<div class="form-row">
					...
					<select name="cidade" class="form-control" style="background-color: rgb(46, 46, 46)">
						...
						<c:forEach items="${cidades.all}" var="cidade">
							<c:choose>
								<c:when test="${cidade.id == filtroCidade.logradouro}">
									<option value="${cidade.id}" selected>${logradouro.nome}</option>
								</c:when>
								<c:otherwise>
				...
				<c:forEach items="${listaLogradouro}" var="logradouro">
					<tr>
						<td id="botoes" width="150px" style="text-align: center">
							<button class="main-btn btn-editar" onclick="location.href='/cidade/preenche-vo?id=${logradouro.id}'"><i
